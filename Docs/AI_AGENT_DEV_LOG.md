...
- Web guild supports independent looping BGM per destination through `TransitionOverlay`: counter starts `counter-theme.mp3` at 5 seconds, table starts `table-theme.mp3` at 50% progress, and shop starts `shop-theme.mp3` at 50% progress. Independent BGM defaults to volume 0.7 and is separate from video audio.
...
- `Assets/Resources/Audio/Guild/`: independent Web guild BGM tracks for counter, table, and shop pages. `web/public/assets/guild/audio/` symlinks to this folder.
