...
- Web prototype implements the guild hall, alpha-tested PNG hotspot hover/click, counter/bulletin/table/shop transition videos, counter page, mock progression/rewards, client letter/reply choices, counter/bulletin/table/shop looping backdrops, table-loop party options, and placeholder pages for board/shop.
...
- RPGUI is installed in `web/` and first-pass styling is applied to the Web guild shell: RPGUI CSS is imported globally, with local overrides that preserve the app layout and readable CJK/system fonts while using RPGUI pixel frames, button art, and cursors for panels and controls.
- Web guild now includes `lobby-to-counter.mp4`, `counter-loop.mp4`, `lobby-to-bulletin.mp4`, `bulletin-loop.mp4`, `lobby-to-table.mp4`, `table-loop.mp4`, `lobby-to-shop.mp4`, and `shop-loop.mp4`; `TransitionOverlay` uses a two-video handoff, starts the post-transition loop shortly before the entry cutscene ends, and guards loop startup with refs so media readiness events cannot repeatedly reset `currentTime` to 0. Counter and bulletin entry/loop audio fade out over the final two seconds, all loop backdrops randomize each loop speed between 0.8x and 1.2x, and transition/loop video audio is routed through Web Audio with 1.3x gain.
- Web guild supports independent looping BGM per destination through the single `GuildAudioProvider` player: counter starts `counter-theme.mp3` at 5 seconds, board starts `board-theme.mp3` two seconds before the entry cutscene ends with a 2-second fade-in, table starts `table-theme.mp3` at 50% progress, and shop starts `shop-theme.mp3` at 50% progress plus 1.5 seconds. Independent BGM defaults to volume 0.5 and is separate from video audio.
...
- RPGUI is the chosen Web guild styling reference/library; RPG CSS was rejected as too small/unproven for this project.
...
- `Assets/Resources/Video/Guild/lobby-to-bulletin.mp4`, `bulletin-loop.mp4`, `lobby-to-table.mp4`, `table-loop.mp4`, `lobby-to-shop.mp4`, `shop-loop.mp4`: Web guild bulletin/table/shop transition and loop videos.
...
- Shop/equipment still opens a placeholder page, but now has a real Web transition video, looping shop backdrop, and independent shop BGM. Counter, notice board, party table, and shop have real Web transition videos and looping video backdrops after entry.
